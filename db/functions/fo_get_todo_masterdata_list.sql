...
BEGIN
    ...
    Return Query
    SELECT array_to_json(array_agg(row_to_json(TodoMasterData))) FROM (
        SELECT (ROW_NUMBER() OVER() :: INTEGER)AS rownumber,(Count(*) Over() :: INTEGER) As totalresultcount,
        SELECT
            TM."TodoId",
        ...
        WHERE CASE WHEN searchparamvalue IS NOT NULL AND searchparamvalue <> '' THEN todomasterlist.searchtext ILIKE '%' || searchparamvalue || '%'
        ELSE True END
        ORDER BY todomasterlist."TodoId" DESC
